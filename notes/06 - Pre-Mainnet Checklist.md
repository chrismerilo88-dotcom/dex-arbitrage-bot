...
- [x] **Owner key security** — validated via a full testnet dry run on Base Sepolia: a 2-of-3 Safe multisig was deployed, and `DexArbitrageBotFlashLoan` ownership was migrated to it through the existing `transferOwnership()`/`acceptOwnership()` flow, with `acceptOwnership()` executed as a properly signed `execTransaction()`, not a single key. See [[03 - Address Registry]] for the addresses and verification steps. **This is a proof the mechanism works, not a mainnet-ready setup** — the current 3 signers include two throwaway keys generated in this session purely to prove the flow; a real deployment needs real, separately-held signer keys (ideally hardware wallets per signer) before this box can be considered actually closed for mainnet.
