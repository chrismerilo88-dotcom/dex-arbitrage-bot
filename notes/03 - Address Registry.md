...
| Contract | Address | Notes |
|---|---|---|
| `DexArbitrageBotFlashLoan` (executor, **current**) | `0x57fdced36481271588d0A5EdAD6F23Df1A4C8716` | v14, live. **Ownership transfer to the Safe is only half-done** -- see the "v14 redeploy" subsection immediately below for the exact pending step. `operator` is `0x0E7de81E4823c69f8c0930b66f718513523abb6b` (unchanged from before), confirmed live via a real `estimateGas` call from that key (reverted with `AmountExceedsCap()`, not `NotOperator()`) |
...
#### v14 redeploy -- Safe ownership transfer left mid-flight, on purpose

Deploying a new executor makes the deployer key its initial owner again -- moving it to the Safe needs the same two-step `transferOwnership()`/`acceptOwnership()` flow as the original migration (see the Safe infrastructure section below), and `acceptOwnership()` must be called *by* the Safe, i.e. a signed 2-of-3 `execTransaction()`. Only one of the three Safe owner private keys is available in this environment (the deployer key, owner #1 of 3) -- the other two live with their respective signers, not in any file here. So the deployer-only half of the migration was completed (`setOperator`, then `transferOwnership(safe)`), confirmed live:

- `owner()` → `0xe9376a141009cF5e6C7CE357Cf595Cf3B6a7a7Aa` (deployer, unchanged for now)
- `pendingOwner()` → `0x63D055A8D26937a98EdA61bb0A4d8C8244de3197` (the Safe)

**Remaining step, needs 2-of-3 Safe signers**: call `acceptOwnership()` on `0x57fdced36481271588d0A5EdAD6F23Df1A4C8716` from the Safe itself -- either via the Safe web UI's "Contract interaction" transaction builder (target the executor address above, function `acceptOwnership()`, no args), or the same `cast wallet sign --no-hash` + concatenated-signature pattern used for the original migration. Until this lands, the deployer key technically still holds full owner-level admin rights on this executor (can call `approveAdapter`, `withdrawToken`, etc.) -- narrower window than before the original Safe migration, but not yet fully closed. The bot itself is unaffected either way: `operator` doesn't depend on who holds `owner`.
