...
- ⚠️ Base Sepolia's new v14 executor has a **2-of-3 Safe multisig transfer left mid-flight**: `transferOwnership()` to the Safe is done, but `acceptOwnership()` needs a second Safe-signer key not available in this environment — see the "v14 redeploy" note in [[03 - Address Registry]] for the exact remaining step. The bot's `operator` role (confirmed live) is unaffected either way. Ethereum Sepolia and Arbitrum Sepolia remain single-key owned.
