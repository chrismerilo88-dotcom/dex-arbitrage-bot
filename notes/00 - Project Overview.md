...
- ✅ **Mainnet: a real executor + adapter are deployed, `maxLoanAmount` permanently 0 — DRY_RUN observation only, no ability to execute a real trade.** `off-chain-bot/` runs continuously (systemd, `WATCH_MODE=true`) against real WETH/USDC liquidity, logging every real quote/gas-estimate outcome to `shared/data/bot.db`. Working toward [[06 - Pre-Mainnet Checklist]]'s explicit bar — 100+ observed opportunities with positive net P&L — before any real trading capital is ever funded. Progress: `node shared/report.js`. See [[03 - Address Registry]]'s Ethereum Mainnet section for addresses and two real bugs this deployment surfaced that pure code review hadn't caught.
